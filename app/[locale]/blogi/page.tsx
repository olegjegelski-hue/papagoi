...
async function getBlogPosts(): Promise<BlogPost[]> {
  try {
    ...
    const response = await fetch(`${baseUrl}/api/notion/blog`, {
      cache: 'no-store',
    })
    if (!response.ok) {
      throw new Error('Notion blog API viga')
    }
    const data = await response.json()
    return data.posts || []
  } catch (error) {
    console.error('Viga blogi päringul:', error)
    return []
  }
}
      ...
      <div className="max-w-6xl mx-auto px-4 py-16">
        ...
        {!hasPosts && (
          <div className="max-w-4xl mx-auto">
            <div className="bg-card/90 backdrop-blur-sm rounded-3xl shadow-2xl p-12 text-center border border-border">
        ...
        {hasPosts && <BlogPostsClient posts={posts} />}
      </div>
    </div>
  )
}
