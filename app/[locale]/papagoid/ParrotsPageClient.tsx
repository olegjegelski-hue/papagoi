...
function getStatusText(status: string, t: (key: string) => string) {
  switch (status) {
    case 'available': return t('statusSeeking')
    case 'sponsored': return t('statusSponsored')
...
const formatDate = (dateString: string, locale: string) => {
  ...
  try {
    ...
    return date.toLocaleDateString(locale === 'ru' ? 'ru-RU' : locale === 'en' ? 'en-GB' : 'et-EE', {
      year: 'numeric',
      month: 'long',
      day: 'numeric'
    })
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          <div className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              >
                <option value="all">{t('filterAllNames')}</option>
                {allNames.map((name) => (
            ...
            <div>
              <select
                ...
                className="w-full px-4 py-3 rounded-full border-2 border-papagoi-beige-300 focus:border-green-500 focus:outline-none focus:ring-2 focus:ring-green-200 bg-papagoi-beige-50"
              >
                <option value="all">{t('filterAllSpecies')}</option>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-16">
          <div className="bg-card rounded-xl p-6 text-center shadow-2xl">
            ...
            <p className="text-gray-600">{t('statParrots')}</p>
          </div>
          <div className="bg-card rounded-xl p-6 text-center shadow-2xl">
            <div className="text-3xl font-bold text-blue-600 mb-2">10+</div>
            <p className="text-gray-600">{t('statSpecies')}</p>
          </div>
          <div className="bg-card rounded-xl p-6 text-center shadow-2xl">
            <div className="text-3xl font-bold text-red-600 mb-2">{baseParrots.filter(p => p.sponsorship?.status === 'available').length}</div>
            <p className="text-gray-600">{t('statSeekingSponsor')}</p>
          </div>
          <div className="bg-card rounded-xl p-6 text-center shadow-2xl">
            <div className="text-3xl font-bold text-yellow-600 mb-2">100%</div>
            <p className="text-gray-600">{t('statLove')}</p>
          </div>
        </div>
        ...
        {(nameFilter !== 'all' || speciesFilter !== 'all' || filter !== 'all') && (
          <div className="text-center mb-6">
            <p className="text-gray-600">
              {t('foundCount', { count: filteredParrots.length })}
            </p>
          </div>
        )}
          ...
          >
                ...
                {false && (
                  <div className="absolute top-4 left-4">
                    <div className={`px-4 py-2 rounded-full text-sm font-medium ${getStatusColor(selectedParrot.sponsorship?.status).bg} ${getStatusColor(selectedParrot.sponsorship?.status).text} ${getStatusColor(selectedParrot.sponsorship?.status).border} border backdrop-blur-sm`}>
                      {getStatusText(selectedParrot.sponsorship?.status, t)}
                    </div>
                  </div>
                )}
              </div>
            ...
            <div className="p-6 md:p-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                  {selectedParrot.birthday && (
                    <div className="flex items-center space-x-2 text-gray-600">
                      ...
                      <div>
                        <span className="font-semibold">{t('birthday')}</span>{' '}
                        {formatDate(selectedParrot.birthday, locale)}
                        {calculateAge(selectedParrot.birthday) !== null && (
                          <span className="ml-2 text-gray-500">
                            ({t('yearsOld', { count: calculateAge(selectedParrot.birthday)! })})
                          </span>
                        )}
                      </div>
                    </div>
                  ...
                  {selectedParrot.gender && (
                    <div className="flex items-center space-x-2 text-gray-600">
                      ...
                      <div>
                        <span className="font-semibold">{t('gender')}</span> {selectedParrot.gender}
                      </div>
                    </div>
                  )}
                  ...
                  {selectedParrot.age && !selectedParrot.birthday && (
                    <div className="flex items-center space-x-2 text-gray-600">
                      ...
                      <div>
                        <span className="font-semibold">{t('age')}</span> {selectedParrot.age}
                      </div>
                    </div>
                  )}
                </div>
              ...
              {selectedParrot.story && (
                <div className="mb-6">
                  <h3 className="text-xl font-bold text-gray-800 mb-3">{t('story')}</h3>
                  {(() => {
              ...
              {selectedParrot.personality && selectedParrot.personality.length > 0 && (
                <div className="mb-6">
                  <h3 className="text-xl font-bold text-gray-800 mb-3">{t('personality')}</h3>
              ...
              {selectedParrot.favorites && selectedParrot.favorites.length > 0 && (
                <div className="mb-6">
                  <h3 className="text-xl font-bold text-gray-800 mb-3">{t('favorites')}</h3>
        ...
        {filteredParrots.length > 0 ? (
                ...
                {false && (
                  <div className="absolute bottom-4 right-4">
                    <div className={`px-3 py-1 rounded-full text-sm font-medium ${getStatusColor(parrot.sponsorship.status).bg} ${getStatusColor(parrot.sponsorship.status).text} ${getStatusColor(parrot.sponsorship.status).border} border backdrop-blur-sm`}>
                      {getStatusText(parrot.sponsorship.status, t)}
                    </div>
                  </div>
                )}
              </div>
          ...
          <div className="text-center py-16">
            <div className="bg-card text-card-foreground border rounded-2xl p-12 max-w-md mx-auto shadow-2xl">
              ...
              <h3 className="text-2xl font-bold text-gray-800 mb-2">{t('noResults')}</h3>
