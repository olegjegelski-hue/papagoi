...
export async function createGiftCardOrder(
  ...
  const ostjaNimiKey =
    findProp(properties, ['Ostja nimi'], 'title') ||
    Object.keys(properties).find((k) => properties[k]?.type === 'title') ||
    null
  const ostjaTelefonKey =
    findProp(properties, ['Ostja telefon'], 'phone_number') ||
    Object.keys(properties).find((k) => properties[k]?.type === 'phone_number') ||
    null
  const ostjaEmailKey =
    findProp(properties, ['Ostja email', 'Ostja e-post'], 'email') ||
    Object.keys(properties).find((k) => properties[k]?.type === 'email') ||
    null
  const ostuKuupaevKey =
    findProp(properties, ['Ostu kuupäev', 'Ostu kuupaev'], 'date') ||
    Object.keys(properties).find((k) => properties[k]?.type === 'date') ||
    null
  const vaartusKey =
    findProp(properties, ['Väärtus', 'Vaartus'], 'number') ||
    Object.keys(properties).find((k) => properties[k]?.type === 'number') ||
    null
  const koodKey =
    findProp(properties, ['Kinkekaardi kood', 'Kood'], 'rich_text') ||
    Object.keys(properties).find((k) =>
      properties[k]?.type === 'rich_text' &&
      (normalizeKey(k).includes('kood') || normalizeKey(k).includes('kinkekaart'))
    ) ||
    null
  const staatusKey =
    findProp(properties, ['Staatus'], 'select') ||
  ...
  if (staatusKey) pageProps[staatusKey] = { select: { name: 'Tellitud' } }
